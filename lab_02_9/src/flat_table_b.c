...
void imp__swap_flat_ptrs(flat_t **flat_1, flat_t **flat_2)
{
    flat_t *temp = *flat_1;
    *flat_1 = *flat_2;
    *flat_2 = temp;
...
void sort_flat_table(flat_table_t *flat_table, sort_key_t key, bool ascending)
{
    ...
    for (unsigned int i = flat_table->size; i > 0; i--)
    {
        ...
        {
            flat_t **flat_1 = flat_table->flat_ptrs + j;
            flat_t **flat_2 = flat_table->flat_ptrs + j + 1;

            int res = imp__flat_comp(*flat_1, *flat_2, key);
            if ((res < 0 && ascending) || (res > 0 && !ascending))
                imp__swap_flat_ptrs(flat_1, flat_2);
        }
    }
}
